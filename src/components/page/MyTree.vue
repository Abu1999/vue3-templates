<template>
  <div class="w-full h-full relative box-border p-4">
    ...
    <div class="mt-2 p-2 select-none overflow-auto" :style="{ maxHeight: props.height }">
      <el-tree ref="treeRef" :data="props.data" highlight-current :default-expand-all="props.defaultExpandAll"
        ...
        :expand-on-click-node="false" :current-node-key="props?.config && props.config.currentNodeKey && props.config.currentNodeKey[props.nodeKey]
          ">
        <template #default="{ node }">
          <div v-if="(node.level == 1 && node.label.length > 9) ||
            (node.level == 2 && node.label.length > 8) ||
            (node.level == 3 && node.label.length > 7)
            ">
