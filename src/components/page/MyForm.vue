      ...
      <template v-for="(item, index) in props.formConfig" :key="index">
        ...
        >
          <el-select
            ...
            :max-collapse-tags="item.collapse ?? 2"
        ...
        <!-- 事件日期选择框 -->
        <el-form-item
          v-else-if="item.type && datePicker.includes(item.type)"
          :prop="item.prop"
          :label="item.label"
          :size="item.size"
          :rules="item.rules ?? setRules(item)"
...
const setRules = (item: FormConfig) => [
  {
    ...
    message:
      item.placeholder ??
      (typeof item.required == 'boolean' ? `请输入${item.label}` : item.required),
    trigger: 'blur'
  }
]
