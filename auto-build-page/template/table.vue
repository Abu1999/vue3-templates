  ...
  <div class="w-full h-full px-5 overflow-auto">
    <"%Class%" v-slot="{ state, commands }">
      ...
      <div class="w-full h-full flex space-x-5">
        <div class="w-auto h-full min-w-[200px]">
          <el-card class="mt-4">
            <div>
              <MyTree :data="[]" @nodeClick="(e: any) => { nodeClick(e, state, commands) }"></MyTree>
            </div>
          </el-card>
        </div>
        <div class="w-full h-full">
          <!-- 查询 -->
          <el-card class="my-4 px-4">
...
import "%Class%" from '@/models/"%dir%"/"%class%"/"%class%".list'
import { tableColumns, formConfig, drawerFormPut, drawerFormPost } from '@/models/"%dir%"/"%class%"/"%class%".config'
import { ref } from "vue";
